...
vi.mock('@tanstack/react-query', async () => {
  ...
  return {
    ...
    QueryClientProvider: ({
      ...
      return React.createElement(actual.QueryClientProvider, { client }, children);
    },
  };
});
...
const mockI18nContext = vi.hoisted(() => {
  ...
  return createContext<{ t: (k: string) => string; currentLanguage: string } | null>(
    null,
  );
});
...
describe('LanguageDirectionSync — via real AppLayout render', () => {
  ...
  it('sets document.lang to the current language code', async () => {
    render(
      <mockI18nContext.Provider value={{ t: (k) => k, currentLanguage: 'fr-CA' }}>
        <App />
      </mockI18nContext.Provider>,
    );
    await waitFor(() => {
  ...
  it('sets ltr for a region-tagged LTR language (en-US)', async () => {
    render(
      <mockI18nContext.Provider value={{ t: (k) => k, currentLanguage: 'en-US' }}>
        <App />
      </mockI18nContext.Provider>,
    );
    await waitFor(() => {
...
describe('queryClient retry policy', () => {
  ...
  function retryFn(): (failureCount: number, error: unknown) => boolean {
    ...
    if (typeof fn !== 'function') throw new Error('retry option is not a function');
    return fn;
  }
