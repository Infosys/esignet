...
vi.mock('react-detect-offline', () => ({
  Detector: ({
    ...
    render: (props: { online: boolean }) => null;
...
describe('AppRouter — background logo variants', () => {
  ...
  it('renders custom background_logo images on the /signin route when configured', async () => {
    ...
    const { container } = renderWithDataRouter('/signin?applicationId=x&authId=y');
    ...
    await waitFor(() => {
      expect(
        container.querySelector('.background-logo') !== null ||
        container.querySelector('.section-background') !== null,
      ).toBe(true);
    });
  });
  ...
  it('renders default background corner images when background_logo is false', async () => {
    ...
    await waitFor(() => {
      expect(
        container.querySelector('.top_left_bg_logo') !== null ||
        container.querySelector('.section-background') !== null,
      ).toBe(true);
    });
  });
