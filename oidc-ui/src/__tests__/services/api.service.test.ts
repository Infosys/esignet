...
import { ApiService, setupResponseInterceptor } from '../../services/api.service';
...
describe('ApiService request interceptor — error handler (line 51)', () => {
  ...
  it('passes the error through when a prior interceptor rejects (covers line 51)', async () => {
    ...
    preInterceptorId = ApiService.interceptors.request.use(
      async () => { throw new Error('pre-interceptor rejected'); },
      undefined,
    );
    ...
    await expect(ApiService.get('/test')).rejects.toThrow('pre-interceptor rejected');
  });
});
  ...
  function buildHandler(navigate: ReturnType<typeof vi.fn>) {
    return (error: unknown): Promise<never> => {
      ...
      if (status && status >= 400) {
        navigate('/something-went-wrong', { state: { code: status } });
      }
