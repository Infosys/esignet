...
import CaptchaRenderer from '../../components/CaptchaComponent/CaptchaRenderer';
import GoogleReCaptcha from '../../components/CaptchaComponent/GoogleReCaptcha';
import CloudflareTurnstile from '../../components/CaptchaComponent/CloudflareTurnstile';
import HCaptcha from '../../components/CaptchaComponent/HCaptcha';
import type { CaptchaFlowComponent } from '../../components/CaptchaComponent/CaptchaModel';
import type { ComponentRenderContext } from '@thunderid/react';

const mockT = vi.fn((key: string) => key);
const mockValidateFieldValue = vi.fn(() => null);
...
let capturedTurnstileProps: Record<string, unknown> = {};
let capturedHcaptchaProps: Record<string, unknown> = {};
...
describe('GoogleReCaptcha component', () => {
  it('passes siteKey to ReCAPTCHA', () => {
    ...
    expect(screen.getByTestId('google-recaptcha').getAttribute('data-sitekey')).toBe('gkey');
  });
  ...
  it('uses fallback values when captcha fields are undefined — covers ?? field branches', () => {
    ...
    const props = {
      captcha: { siteKey: undefined, theme: undefined, size: undefined } as Parameters<typeof GoogleReCaptcha>[0]['captcha'],
      handleSuccess: vi.fn(),
      handleError: vi.fn(),
      handleExpire: vi.fn(),
    };
    render(<GoogleReCaptcha {...props} />);
    expect(screen.getByTestId('google-recaptcha').getAttribute('data-sitekey')).toBe('');
  });
...
describe('CloudflareTurnstile component', () => {
  ...
  it('uses fallback values when captcha fields are undefined — covers ?? field branches', () => {
    const props = {
      captcha: { siteKey: undefined, theme: undefined, size: undefined } as Parameters<typeof CloudflareTurnstile>[0]['captcha'],
      handleSuccess: vi.fn(),
      handleError: vi.fn(),
      handleExpire: vi.fn(),
    };
    render(<CloudflareTurnstile {...props} />);
    expect(screen.getByTestId('cloudflare-turnstile').getAttribute('data-sitekey')).toBe('');
  });
...
describe('HCaptcha component', () => {
  it('passes siteKey to HCaptchaLib', () => {
    ...
    expect(screen.getByTestId('hcaptcha').getAttribute('data-sitekey')).toBe('hkey');
  });

  it('uses fallback values when captcha fields are undefined — covers ?? field branches', () => {
    const props = {
      captcha: { siteKey: undefined, theme: undefined, size: undefined } as Parameters<typeof HCaptcha>[0]['captcha'],
      handleSuccess: vi.fn(),
      handleError: vi.fn(),
      handleExpire: vi.fn(),
    };
...
describe('CaptchaComponent callbacks', () => {
  ...
  it('handleSuccess calls onInputChange with the token', () => {
    ...
    onChange('test-token');
    expect(context.onInputChange).toHaveBeenCalledWith('captcha-field', 'test-token');
  });

  it('handleSuccess with null token calls onInputChange with empty string', () => {
    const context = makeContext();
    render(<CaptchaComponent component={makeComponent()} context={context} />);
    const onChange = capturedRecaptchaProps.onChange as (t: string | null) => void;
    onChange(null);
    expect(context.onInputChange).toHaveBeenCalledWith('captcha-field', '');
  });
